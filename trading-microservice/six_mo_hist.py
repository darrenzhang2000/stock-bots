...
for day in range(1100, len(G_stock)):
    ...
    for row in fall_dataframe.index:
        ...
        payload=f"email=testuser%40gmail.com&ticker={fall_dataframe.loc[row,'Ticker']}&quantity={orig_stock}&action={action}&dateTime={A_stock_xl.loc[day, 'Date']}&reason={given_reason}&price={fall_dataframe.loc[row, 'Price']}"
        headers = {
    ...
    for row in stable_dataframe.index:
        if stable_dataframe.loc[row, 'Decision'] == 1:
            ...
            url = "http://localhost:8000/transactions/"
            #put in transactions
            payload=f"email=testuser%40gmail.com&ticker={stable_dataframe.loc[row, 'Ticker']}&quantity={stock_in_half}&action={action}&dateTime={A_stock_xl.loc[day, 'Date']}&reason={given_reason}&price={stable_dataframe.loc[row, 'Price']}"
            headers = {
            'Content-Type': 'application/x-www-form-urlencoded'
            }
            ...
            url = "http://localhost:8000/transactions/"
            payload=f"email=testuser%40gmail.com&ticker={stable_dataframe.loc[row, 'Ticker']}&quantity={orig_stock}&action={action}&dateTime={A_stock_xl.loc[day, 'Date']}&reason={given_reason}&price={stable_dataframe.loc[row, 'Price']}"
            headers = {
            'Content-Type': 'application/x-www-form-urlencoded'
            }
    ...
    if(len(grow_dataframe.index) != 0 and liquid_cash != 0):
        ...
        for row in grow_dataframe.index:
            ...
            url = "http://localhost:8000/transactions/"
            payload=f"email=testuser%40gmail.com&ticker={grow_dataframe.loc[row, 'Ticker']}&quantity={new_amount_to_buy}&action={action}&dateTime={A_stock_xl.loc[day, 'Date']}&reason={given_reason}&price={grow_dataframe.loc[row, 'Price']}"
            headers = {
            'Content-Type': 'application/x-www-form-urlencoded'
            }
